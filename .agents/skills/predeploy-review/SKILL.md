...
description: Perform a pre-deployment multi-dimensional review with adversarial verification. Use for /predeploy-review-style final release checks.
---
...
> **이 스킬의 핵심 가치 = 적대적 검증.** 리뷰어는 위험을 과장하는 경향이 있다(가드/게이팅된 코드를 CRITICAL로 보고). 검증 단계가 이를 걸러내지 않으면 신뢰할 수 없는 공포 리스트가 된다. 검증을 절대 생략하지 말 것.

`사용자 요청/추가 컨텍스트` — 선택 인자:
...
2. **근거 없는 발견 금지** — 모든 발견은 실제로 파일 읽기/검색으로 읽은 코드에 근거하고 `파일:줄번호`를 명시한다. 일반론·추측 금지.
...
5. **프로젝트 컨벤션 기준** — `AGENTS.md`와 기존 `CLAUDE.md`/`.claude/rules/`가 있으면 해당 파일들을 판단 기준으로 삼는다(try-catch+Custom Exception, AppColors/AppTextStyles 상수, ARB i18n, isDarkMode prop, ScreenUtil 등). 일반 베스트프랙티스가 아니라 이 프로젝트 규칙으로 본다.
...
**Workflow 도구를 사용할 수 있으면(이 스킬 실행 자체가 opt-in) Workflow로 실행한다.** 불가하면 3단계의 Task 폴백을 사용한다.
...
5. `사용자 요청/추가 컨텍스트`가 `global`이면 i18n(en/ja) 결함을 must_fix로 승격, 경로 인자면 해당 영역 집중.
