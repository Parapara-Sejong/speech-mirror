---
name: rp
description: Generate an implementation report and PR description in parallel from shared git context. Use for /rp-style combined report and PR body requests.
---
...
- `prompt`: 아래 템플릿
- 반드시 Skill 도구로 `report` 스킬 호출 또는 `.agents/skills/report/SKILL.md`의 지침을 그대로 따라 `.report/`에 파일 생성
...
# 작업
`.agents/skills/report/SKILL.md`에 정의된 Report Mode 지침을 그대로 따라
...
# 작업
`.agents/skills/pr-description/SKILL.md`에 정의된 PR Description Mode 지침을 따라
